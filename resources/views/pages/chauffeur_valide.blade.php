	...
	<script type="text/javascript">
		...
		function validerUnChauffeur(id) {
			Swal.fire({
				...
				text: "Vous êtes en train de vouloir désactiver ce chauffeur ! Assurez-vous que c'est bien le bon !",
				icon: 'warning',
		...
		function getproducts() {
			var database = firebase.firestore();
			var ref = database.collection('users').where("status", "==", true).where("role", "==", "Drivers").limit(500)
					...
					if (querySnapshot.empty == false) {
						...
						querySnapshot.forEach((doc) => {
							$("#products-list").append('\'<tr class="nk-tb-item">' +
								...
								'</td>' +
								'<td class="nk-tb-col tb-col-mb">' +
								'<span class="badge badge-outline-success">Valide</span>' +
								'</td>' +
